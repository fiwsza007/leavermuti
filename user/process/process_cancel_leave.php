...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ดึงข้อมูลใบลา
    $sql_leave = "SELECT leavestart, leavestatus FROM leaves WHERE leavesid = ? AND employeesid = ?";
    ...
    // ตรวจสอบสถานะใบลา
    if (!in_array($leave['leavestatus'], ['รอหัวหน้าอนุมัติ', 'รอรองผ.อ.อนุมัติ', 'รอผ.อ.อนุมัติ', 'อนุมัติ','รออนุมัติ'])) {
        echo json_encode([
            'status' => 'error',
    ...
    if($leave['leavestatus']=='อนุมัติ'){
        $sql_update = "UPDATE leaves SET note = 'ยกเลิกลาและคืนวันลา',leavestatus = 'รออนุมัติ'  WHERE leavesid = ?";
        $stmt_update = $conn->prepare($sql_update);
    }
    else{
        $sql_update = "UPDATE leaves SET note = 'ยกเลิกลา' WHERE leavesid = ?";
        $stmt_update = $conn->prepare($sql_update);
    }
    ...
    $stmt_update->bind_param('i', $leavesid);

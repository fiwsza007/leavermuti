...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) {
        ...
        $sql = "INSERT INTO employees (prefix, fname, lname, email, password, department, position, staffstatus, startwork, pic) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssssiiiss", $prefix, $firstname, $lastname, $email, $password, $subdepart, $position, $status, $start_date, $new_filename);
